...
public interface AnswerRepository extends JpaRepository<AnswerEntity, Long>{
	...
	//@Query("select u.userName from User u inner join u.area ar where ar.idArea = :idArea")
	 @Query(value = "select ga.question_answer, ga.given_ansid, ga.question_id,  ea.answerid, ea.student_id, ea.start_time, ea.end_time, ea.clientid, ea.exam_id, qa.question_answer from given_answer_entity ga join answer_entity ea ON ga.answerid = ea.answerid join question_entity qa ON qa.question_id = ga.question_id where ea.exam_id = 1 and ea.clientid = 1 and ea.student_id = 1202", nativeQuery = true)
